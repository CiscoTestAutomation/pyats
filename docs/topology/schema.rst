...
.. _topology_kleenex_integration:

Kleenex Integration
-------------------

In the above schema, each device may specify an optional ``clean:`` block.
This block should be used to store **static** device-clean information,
a subset of those described in the :ref:`Kleenex Clean File <clean_file>`.

As the clean input file is used as a supplement to topology files, the use of
``clean:`` block should allow users to drastically reduce the size of their
clean file, moving static information such as rommon variables & static IPs
into the testbed file instead, and only keeping volatile data in the clean
file.

Any key value pairs defined in a device's ``clean:`` block are first parsed
into its ``Device.clean`` dictionary.  When Kleenex loads the clean file
and applies it to each Device object, if the same keys are present, they
are overwritten, following a *nested dictionary update* scheme.
...
.. code-block:: yaml
    ...
    # -------
    #
    #   testbed yaml file with device clean block
    ...
    devices:
        my-device:
            ...
            # only keep static information here
            # ---------------------------------
            clean:
                golden_image: "/path/to/golden/image.bin"
                preclean: |
                    switchname my_device
                    power redundancy-mode combined force
                    username admin password Cisc0123 role network-admin
...
Kleenex cleaners may also extend device.clean with metadata discovered
on the device during the clean.

For more information on how Kleenex operates, refer to :ref:`kleenex_index`.
